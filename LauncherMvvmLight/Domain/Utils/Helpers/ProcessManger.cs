...
namespace System.Diagnostics
{
    ...
    /// </devdoc>
    /// <internalonly> 
    public static class WinProcessManager
    {
    ...
    /// </devdoc>
    /// <internalonly> 
    public static class NtProcessManager
    {
            ...
            try
            {
                ...
                for (; ; )
                {
                    ...
                    try
                    {
                        ...
                        // So we just to try avoid the race by calling the same method 10 (an arbitary number) times. 
                        //
                        for (int i = 0; i < 10; i++)
                        {
    ...
    public static class NtProcessInfoHelper
    {
...
#pragma warning disable 169
        public static ProcessInfo[] GetProcessInfos()
        {
            ...
            // This buffer is large and creating a large pinned object should be avoided
            ProcessInfo[] processInfos;

            // The process names will be pointing to the data blocks returned by
            // NtQuerySystemInformation. So don't use managed buffer for this unless
            // you pinned the buffer all the time. Otherwise NamePtr might be pointing 
            // to nowhere.
            // I also found AllocHGlobal is a little bit faster. 
            ...
            try
            {
                ...
                processInfos = GetProcessInfos(dataPtr);
            }
            finally
            {
                Marshal.FreeHGlobal(dataPtr);
        ...
        internal unsafe static string GetProcessShortName(char* name, int length)
        {
            ...
            if (*pSlash == '\\')
            { // if pSlash is pointing to a slash, then
