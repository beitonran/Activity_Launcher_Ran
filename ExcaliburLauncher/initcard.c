...
#include "ExcUnetMs.h"
#define B2DRMHANDLE ((int) (handlepx))

static struct exc_bc *allocBank = 0;
...
int InitM4K(int handlepx, BOOL resetFlag);
...
{
	int	i,
		iError,						/* intermediate error routine passed from
	...
	*/
	for (i=0; i<NUMCARDS; i++)
	{
		if (cardpx[i].allocated == 0)
		{
			...
			cardpx[i].module = module_num;  /* this will later be modified for PCMCIA cards */
			handlepx = i;
			g_handlepx = i;
			break;
		}
	...
	*/
	...
	/*
	   Initialize the "Bridge to device raw memory" on the predefined REMOTE Px device (dev=888) 
	   or on the given "device_num" in case and the b2drmPxDeviceIsRemote finds on the registry 
	   that it is a USB or NET device
	*/
//	b2drmStartDebug(handlepx);
	if (b2drmDeviceIsRemote(device_num, handlepx))
	{
		int sizeof_exc_bc;
		sizeof_exc_bc = sizeof(struct exc_bc);

		cardpx[handlepx].isUNet = TRUE;
		cardpx[handlepx].originalModuleNumber = module_num;
		...
		b2drmSetBaseAddress((unsigned int) & (cardpx[handlepx].exc_bd_bc->stack[0]), handlepx );

		// we have a REMOTE access device, so we init access to the device

		iError = b2drmInitRemoteAccessToDevice( device_num, module_num, handlepx );
		if (iError < 0)
			return (iError);
		...
		cardpx[handlepx].isUnetMultiMsgReadSupported = b2drmIsCmdClearEOMStatusSupported(device_num);
		...
		if (resetFlag == TRUE)
		{
			...
			//	{
				...
				// RESET_DELAY 25*100
				while(EndTimer_Px(startime, RESET_DELAY) != 1)
				{
			...
			if (efound == 0) {
				b2drmReleaseRemoteAccessToDevice( cardpx[handlepx].card_addr, cardpx[handlepx].originalModuleNumber, handlepx);
				return(etimeoutreset);
			}
		} // end ResetFlag == TRUE
			// At this stage the remote access to the PX  has been established

	}// end of 888 USB and NET device 
	else

	{ // SIMULATE & regular non-unet module
		...
		*/
		if (device_num == SIMULATE)
		{
			...
			*/
			...
			cardpx[handlepx].intnum = 1;						/* interrupt 1 (at least a flag) */
		...
		else // regular non-unet module
		{
			...
			boardtype = 0;
			Get_4000Board_Type(cardpx[handlepx].card_addr, &boardtype);

			if (boardtype == EXCARD_BRDTYPE_1553PX)
			{
			...
			{
				iError = InitM4K(handlepx, resetFlag);
			}
		...
		} // end of regular non-unet module
	} // end of SIMULATE & regular non-unet module
...
#ifdef _WIN32
		/*  wait for reset to complete (RESET_DELAY is 25*100)			*/
		StartTimer_Px(&startime);
		while (EndTimer_Px(startime,RESET_DELAY) != 1)
		{
...
//	if (cardpx[handlepx].exc_bd_bc->pcmciaEPorPx == 'P') /* NIOS PCMCIA-Px */
	if ( RBYTE_D2H( cardpx[handlepx].exc_bd_bc->pcmciaEPorPx ) == 'P') /* NIOS PCMCIA-Px */
	{
		isPCMCIApx = 1;
		cardpx[handlepx].processor = PROC_NIOS;
	...
	Since this function is dependent on a Windows API call, it is not portable to other operating systems as written.	*/
{
	...
	iError = Get_4000Module_Type(cardpx[handlepx].card_addr, cardpx[handlepx].module, &modtype);
	if (iError)
	{
	...
	**************************************************************************** */
	...
	if((modtype  & ~(EXC4000PCI_MODULETYPE_32BITACCESS_FLAG)) != EXC4000_MODTYPE_MMSI)

#else
	// if(modtype != EXC4000_MODTYPE_PX)
	if((modtype  & ~(EXC4000PCI_MODULETYPE_32BITACCESS_FLAG)) != EXC4000_MODTYPE_PX)
#endif
	{
...
// to enable Host2Dpr-like debugs on local modules
	...
	b2drmStartLocalAccessToPx(handlepx);
...
int borland_dll Release_Module_Px(int handlepx)
{
	...
	if (cardpx[handlepx].allocated == 1)
	{
		...
		// Isaac: check if this is a remote device. Note: "card_addr" saves the Device number
		if( b2drmDeviceIsRemote( cardpx[handlepx].card_addr, handlepx ) ){
			b2drmReleaseRemoteAccessToDevice( cardpx[handlepx].card_addr, cardpx[handlepx].originalModuleNumber, handlepx);
...
//	if (b2drmDeviceIsRemote(cardpx[handlepx].card_addr, handlepx))
	if (b2drmIsRemoteAccessFlag(handlepx) == TRUE)
	{
